...
public class Main_69 {
    ...
    private static int solution(int[][] arr) {
        ...
        for(int i=1 ; i<N ; i++) {
            dp[0][i] = arr[0][i] + Math.max(dp[1][i-1], dp[2][i-1]);   // pattern 0
            dp[1][i] = arr[1][i] + Math.max(dp[0][i-1], 
            				Math.max(dp[2][i-1], dp[3][i-1]));    // pattern 1
            dp[2][i] = arr[2][i] + Math.max(dp[0][i-1], dp[1][i-1]);   // pattenr 2
            dp[3][i] = arr[0][i] + arr[2][i] + dp[1][i-1];   // pattern 3
        }

        return Math.max(Math.max(dp[0][N-1], dp[1][N-1]), 
        			Math.max(dp[2][N-1], dp[3][N-1]))  // 4개의 경우 중 가장 큰 경우 반환
    }
